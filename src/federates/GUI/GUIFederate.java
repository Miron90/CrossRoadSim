...
//----------------------------------------------------------
    ...
    public void runFederate( String federateName ) throws Exception
    {
        ...
        // send an interaction.
        for(int i=0;i<CASH_QUEUE_QUANTITY;i++){
            ObjectInstanceHandle cashObjectHandle = rtiamb.registerObjectInstance(cashHandle);
            GUIList.add(new GUI(GUIList.size(),cashObjectHandle));
            log("Registered Queue, handle=" + cashObjectHandle);
        }
        while( fedamb.federateTime<SIM_TIME )
        {
            ...
            for(int i=0;i<CASH_QUEUE_QUANTITY;i++) {
                System.out.println("min time to serve "+ GUIList.get(i).getTimeOfServing());
                if(GUIList.get(i).getTimeOfServing()!=-1 && minTime> GUIList.get(i).getTimeOfServing() ){
                    minTime= GUIList.get(i).getTimeOfServing()+0.5;
                }
            }
...
//		log( "Deleted Object, handle=" + objectHandle );
    ...
    private void updateCash(GUI GUI) throws NotConnected, FederateNotExecutionMember, ObjectInstanceNotKnown, RestoreInProgress, AttributeNotOwned, AttributeNotDefined, SaveInProgress, RTIinternalError {
        AttributeHandleValueMap attributes = rtiamb.getAttributeHandleValueMapFactory().create(3);

        HLAinteger32BE cashId = encoderFactory.createHLAinteger32BE( GUI.getCashId());
        attributes.put( this.cashIdHandle, cashId.toByteArray() );

        HLAinteger32BE queueId = encoderFactory.createHLAinteger32BE(GUI.getQueueId() );
        attributes.put( this.cashQueueIdHandle, queueId.toByteArray() );

        HLAinteger32BE quantityOfServed = encoderFactory.createHLAinteger32BE( GUI.getServed() );
        attributes.put( this.quantityOfServedHandle, quantityOfServed.toByteArray() );

        rtiamb.updateAttributeValues( GUI.getId(), attributes, generateTag() );
    }
